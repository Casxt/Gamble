...
public class MsgHandle {
    ...
    private void LoginNotify(JSONObject Msg) {
        System.out.println(String.format("玩家 %s 加入游戏", Msg.getString("Name")));
    }

    private void GamblerJoinNotify(JSONObject Msg) {
    ...
    private void GambleNumNotify(JSONObject Msg) {
        System.out.println(String.format("%s点！", Msg.getString("Msg")));
    }

    private void GambleResultNotify(JSONObject Msg) {
    ...
    private void GambleUserChipEmptyNotify(JSONObject Msg) {
    ...
    private void GambleServerChipEmptyNotify(JSONObject Msg) {
        System.out.println("庄家运气怎么这么差，竟然输光了，掀桌子不玩儿了！大家散场啦！");
    }
}
